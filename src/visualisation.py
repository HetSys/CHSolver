...
def plot_conc_evol(data_obj, animation_fps=10, ti=0, tf=-1):
    ...
    for k in range(ti_index, tf_index+1):
        ...
        im.append(shw)
        im.append(plt.title('Concentration Evolution',
    ...
    ax.set_axis_off()
    fig.add_axes(ax)
    final_an = anim.ArtistAnimation(
        fig, ims, interval=5, repeat=True, blit=False)
    final_an.save('Conc_Evolution.gif',
                  writer=anim.PillowWriter(fps=animation_fps))
...
def plot_free_energy(data_obj, ti=0, tf=-1):
    ...
    for t in range(num_checkpoints):
        ...
        for i in range(grid_res):
            for j in range(grid_res):
                # dV = 1? so no term in here
                F[t] += f_c[t, i, j] + 0.5*K*grad_c_sqd[i, j]
    # Divided by total volume, since each dV is 1, just equal to number of cells
    F = F/(grid_res**2)
